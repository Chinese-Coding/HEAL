...
class PointPillarUncertaintyLoss(PointPillarLoss):
    ...
    def forward(self, output_dict, target_dict, suffix=""):
        ...
        reg_preds_w_angle, reg_targets_w_angle = self.add_sin_difference_and_angle(reg_preds,
                                                                                   reg_targets)  # note the difference
        reg_loss = weighted_smooth_l1_loss(reg_preds_w_angle[..., :7], reg_targets_w_angle[..., :7],
    ...
    def logging(self, epoch, batch_id, batch_len, writer=None):
        ...
        logger.info("[epoch %d][%d/%d] || Loss: %.4f || Conf Loss: %.4f"
                    " || Loc Loss: %.4f || Dir Loss: %.4f || IoU Loss: %.4f || Unc Loss: %.4f" % (
                        epoch, batch_id + 1, batch_len,
                        total_loss, cls_loss, reg_loss, dir_loss, iou_loss, unc_loss))
...
class KLLoss(nn.Module):
    ...
    def kl_loss_angular(diff, s, lambda_V=1, s0=1, limit_period=False):
        ...
        if limit_period:
            ...
            loss = loss = torch.log(mathh.i0e_cuda(exp_minus_s) * torch.exp(
                exp_minus_s)) - exp_minus_s * cos_abs.detach() + lambda_V * F.elu(s - s0)
        else:
            loss = torch.log(mathh.i0e_cuda(exp_minus_s) * torch.exp(exp_minus_s)) - exp_minus_s * torch.cos(
                diff) + lambda_V * F.elu(s - s0)
