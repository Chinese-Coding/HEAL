...
class PixorLoss(nn.Module):
    ...
    def forward(self, output_dict: dict, target_dict: dict):
        """
    ...
    def logging(self, epoch, batch_id, batch_len, writer=None):
        ...
        print("[epoch %d][%d/%d], || Loss: %.4f || cls Loss: %.4f || reg Loss: %.4f" % (
            epoch, batch_id + 1, batch_len, total_loss.item(), cls_loss.item(), reg_loss.item()))
