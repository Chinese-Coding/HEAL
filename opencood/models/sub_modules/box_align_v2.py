...
def vis_pose_graph(poses, pred_corner3d, save_dir_path, vis_agent=False):
    ...
    for iter, pose in enumerate(poses):
        ...
        pred_corners3d_in_ego = [box_utils.project_box3d(pred_corner3d[i], relative_t_matrix[i]) for i in
                                 nonempty_indices]
        ...
        for agent_id in range(len(pred_corners3d_in_ego)):
            ...
            for i in range(corner2d.shape[0]):
                ...
                plt.plot(corner2d[i, [0, 1, 2, 3, 0], 0], corner2d[i, [0, 1, 2, 3, 0], 1], linewidth=1,
                         c=COLOR[agent_id])
...
def all_pair_l2(A, B):
    ...
    C = np.sqrt(np.sum(A * A, 1, keepdims=True).repeat(TwoAB.shape[1], axis=1) \
                + np.sum(B * B, 1, keepdims=True).T.repeat(TwoAB.shape[0], axis=0) - TwoAB)
    return C


def box_alignment_relative_sample_np(pred_corners_list, noisy_lidar_pose, uncertainty_list=None, landmark_SE2=True,
                                     adaptive_landmark=False, normalize_uncertainty=False, abandon_hard_cases=False,
                                     drop_hard_boxes=False, drop_unsure_edge=False, use_uncertainty=True, thres=1.5,
                                     yaw_var_thres=0.2, max_iterations=1000):
    """
    Perform box alignment for one sample. Correcting the relative pose.
    TODO: 有意思, 这个函数这么多参数, 而在模型传递参数时全部用的是 dict, 这不是很矛盾吗? 可能不是同一个人写的吧......
    Args:
    ...
    nonempty_indices = [idx for (idx, corners) in enumerate(pred_corners_list) if
                        len(corners) != 0]  # if one agent detects no boxes, its corners is just [].

    pred_corners_world_list = \
        [box_utils.project_box3d(pred_corners_list[i], lidar_pose_noisy_tfm[i]) for i in
         nonempty_indices]  # [[N1, 8, 3], [N2, 8, 3],...]
    pred_box3d_list = \
        [box_utils.corner_to_center(corner, order) for corner in pred_corners_list if
         len(corner) != 0]  # [[N1, 7], [N2, 7], ...], angle in radian
    pred_box3d_world_list = \
        [box_utils.corner_to_center(corner, order) for corner in
         pred_corners_world_list]  # [[N1, 7], [N2, 7], ...], angle in radian
    pred_center_list = \
    ...
    pred_center_allpair_dist = all_pair_l2(pred_center_world_cat,
                                           pred_center_world_cat)  # [sum(pred_box), sum(pred_box)]
    ...
    for box_idx in range(cum):
        ...
        cluster_dict[cluster_id]['box_center_world'] = [pred_center_world_cat[idx] for idx in
    ...
    for b in record_len:
        refined_lidar_pose.append(
            box_alignment_relative_sample_np(
                pred_corner3d_list[start_idx: start_idx + b], lidar_poses[start_idx: start_idx + b],
