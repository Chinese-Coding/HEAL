...
class CamEncode(nn.Module):  # 提取图像特征进行图像编码
    ...
    def forward(self, x):
        ...
        features = self.get_eff_features(
            x_img_)  # depth: B*N x D x fH x fW(24 x 41 x 8 x 22)  x: B*N x C x D x fH x fW(24 x 64 x 41 x 8 x 22)
...
class CamEncode_Resnet101(nn.Module):  # 提取图像特征进行图像编码
    ...
    def forward(self, x):
        ...
        features = self.get_resnet_features(
            x_img)  # depth: B*N x D x fH x fW(24 x 41 x 8 x 22)  x: B*N x C x D x fH x fW(24 x 64 x 41 x 8 x 22)
