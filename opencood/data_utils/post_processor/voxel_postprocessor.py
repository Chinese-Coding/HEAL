...
class VoxelPostprocessor(BasePostprocessor):
    ...
    def generate_anchor_box(self):
        ...
        w = self.params['anchor_args']['w']
        h = self.params['anchor_args']['h']
        r = self.params['anchor_args']['r']  # 调试值：[0, 90], 指的是生成狂的框的角度吗？
        ...
        """
        调试：anchors.shape = [128, 256, 2, 7], 具体来看就是, 总共生成了 125 * 256 个点, 每个点上有两个框, 每个框又由 7 个元素组成
        而且, 只要配置文件相同, 每次生成的框应该是一模一样的
        """
        return anchors
