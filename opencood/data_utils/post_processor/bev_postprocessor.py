...
class BevPostprocessor(BasePostprocessor):
    def __init__(self, anchor_params: Mapping, train: bool):
    ...
    def post_process(self, data_dict, output_dict):
        ...
        for cav_id, cav_content in data_dict.items():
            ...
            if mask.sum() > 0:
                ...
                # (number of high confidence bbx, 4, 2)
                projected_boxes2d = \
                    box_utils.project_points_by_matrix_torch(box3d.view(-1, 3), transformation_matrix)[:, :2]
    ...
    def reg_map_to_bbx_corners(self, reg_map, mask):
        ...
        grid_x = torch.arange(self.geometry_param["L1"], self.geometry_param["L2"],
                              grid_size, dtype=torch.float32, device=device)
        grid_y = torch.arange(self.geometry_param["W1"], self.geometry_param["W2"],
                              grid_size, dtype=torch.float32, device=device)
